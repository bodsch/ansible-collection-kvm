...
class LibvirtMonolitic(LibvirtService):
    """
    Manage the monolithic libvirt daemon (libvirtd service and its sockets).

    Typically used to disable the monolithic units when switching a host over to
    the modular daemons, or to enable/start libvirtd on older hosts. The unit
    list is supplied by the role; this module only applies the requested
    lifecycle ``state`` to those units.
    """
